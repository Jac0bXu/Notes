...
\begin{itemize}
    ...
    \item Libraries: premade importable modules with code to fulfill certain functions used in many places.
\end{itemize}
...
When a module refers to another module it is instantiating it. These modules are defined hierarchically. Higher level modules can instantiate lower level modules as many times as desired. Higher level modules may instantiate the same lower level module.\\
...
\end{lstlisting}
There are words with special meaning, and cannot be used as names.
\begin{itemize}
    \item \defn{input}{signal that is an input to the module}
    \item \defn{output}{output signal, cannot be read within module}
