...
\end{center}
\section{Dimensions of Linear Transformations}
We already know the definitions of
\section{Eigenvectors and Eigenvalues}
\defn{Eigenvector}{for an $n\times n$ matrix \textbf{A}, is a non-zero vector such that $\mathbf{Ax=}\lambda\mathbf{x}$ for some scalar $\lambda$.}\\
\defn{Eigenvalue}{scalar value mentioned in the above definition: $\lambda$.}\\
The steps to finding these are:
\begin{enumerate}
    \item Find all values $\lambda$ such that $\det(\mathbf{A-I}\lambda)=0$. These are
          your eigenvalues.
    \item For each $\lambda$, find the Null Space of $\mathbf{A-I}\lambda$, that is, find
          the non-zero vector such that $(\mathbf{A-I}\lambda)\mathbf{x=0}$. Each of
          these will be your eigenvectors.
\end{enumerate}
\defn{Eigenspace}{for an $n\times n$ matrix \textbf{A}, is the subspace of $\mathbb{R}^n$ that contains all the eigenvectors of \textbf{A} and $\mathbf{0}$. It is denoted by $E_\lambda$}.
\begin{equation*}
    E_\lambda = \{\mathbf{v|Av =}\lambda\mathbf{v}\} = \{ \mathbf{v}|(\mathbf{A}-\lambda\mathbf{I})\mathbf{v=0} \}
\end{equation*}
\begin{center}
    \textbf{Theorem.} The eigenvalues of a triangular matrix are its main diagonal.
\end{center}
\begin{center}
    \textbf{Theorem.} $0$ is an eigenvalue if and only if the determinant of the matrix is already $0$
\end{center}
\begin{center}
    \textbf{Theorem.} If all eigenvalues are distinct, the corresponding eigenvectors will be linearly independent.
\end{center}
\defn{Characteristic equation}{$\det(\mathbf{A}-\lambda\mathbf{I})=0$}\\
The characteristic equation is an $n$ degree polynomial with $n$ roots, taking into account the multiplicities of the roots themselves.\\
\defn{Algebraic Mulitplicity}{Number of times a root appears in the characteristic equation.}\\
\defn{Geometric Multiplicity}{Dimension of the eigenspace corresponding to an eigenvalue.}\\
\defn{Similarity}{Two matrices \textbf{A,B} are similar if $\exists \mathbf{P| (P^{-1}AP = B) \lor (PBP^{-1} = A)}$}\\
\begin{center}
    \textbf{Theorem. }Similar matrices have the same characteristic equation and the same eigenvalues with the same multiplicities.
\end{center}
\textit{Note: Row equivalence is different from similarity. Matrix operations modify eigenvectors and eigenvalues.}\\
\section{Diagonalization}
\begin{center}
    \textbf{Theorem. }$\mathbf{A = QBQ^{-1}\implies A^n = QB^n Q^{-1}}$
\end{center}
A triangular matrix will have eigenvalues equal to the values on the diagonal, and any matrix similar to it will have the same eigenvalues.\\
\begin{center}
    \textbf{Theorem. }For a diagonal matrix $\mathbf{D}$, $\mathbf{D^k}$ will be the same as the original matrix with each of the terms raised to the $k^{th}$ power. 
\end{center}
\begin{center}
    \textbf{Theorem. }$\mathbf{A = PDP^{-1}\implies AP = PD}$
\end{center}
\begin{center}
    \textbf{Theorem. }$\mathbf{P}$ is formed by all eigenvectors, while \textbf{D} is formed by all eigenvalues.
\end{center}
\end{document}