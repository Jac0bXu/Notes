...
List-Delete(header, x):
    ...
    header->tail  =  x->prev
\end{lstlisting}
As can be observed, the process involves a lot of back and forth, specially since we need to verify the existence of several nodes node.\\
...
Let $c = \log_b(a)$.\\ This means that the number of leaf  nodes will be $n^c$. \\ Since $T(1) = \Theta(1)$, leaf node computation will be $\Theta(n^c)$.\\
